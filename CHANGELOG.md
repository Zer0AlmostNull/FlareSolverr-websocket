...
* **Protocol-Aware Deduplication & Batch Decomposition (`frame_router.py`)**:
  ...
  - Added hash-suffix keying for MevX `subscribeFlashPool` notifications (`flash:{poolAddress}:{createdAt}:{sha256(payload)[:8]}`) to eliminate collisions during rapid block activity in the same integer second.
  - Implemented cross-process deduplication (`_same_message_cross_process`) matching keys within a 3.0s wall-clock arrival timestamp window with zero-drop keyless fallbacks.
* **Frame Classification & CDP Crash Hooks (`chrome_manager.py`)**:
  - Added `last_data_frame_ts`, `last_control_frame_ts`, `consecutive_stalls`, and rolling 60s `data_frame_rate()` / `control_frame_rate()` to `TabState`.
  - Registered CDP `Inspector.Detached` and `Target.TargetCrashed` handlers to immediately flag dead renderer tabs.
  - Implemented Tier 1 in-place soft tab reload (`soft_reload_tab()`) with network domain re-enablement.
* **Zero-Drop Dual-Process Standby Chrome Recycling (`chrome_manager.py`)**:
  - Implemented `launch_standby_browser()`, `warm_standby_tabs()` with staggered concurrency (`concurrency=2`) to strictly protect container memory limits, and `swap_standby_browser()` with atomic CAS pointer swaps, cross-process buffer deduplication, 5s trailing frame drain, and old browser tree termination.
* **3-Tier Stall Escalation & Maintenance Scheduler (`flaresolverr.py`)**:
  - Implemented 3-Tier Escalation engine (Tier 1 Soft Reload -> Tier 2 Shadow Tab Handoff -> Tier 3 Standby Chrome Swap) with 180s GMGN headless warmup grace period.
  - Added periodic 6-hour and 1.2GB Chrome RSS standby recycling triggers with emergency in-place fast restart fallback at 1.6GB.
* **Prometheus Observability (`metrics.py`)**:
  - Added `WS_DATA_FRAME_RATE`, `WS_CONTROL_FRAME_RATE`, `WS_STALL_ESCALATIONS`, `WS_STANDBY_BROWSER_RECYCLES`, and `WS_STANDBY_HANDOFF_DURATION`.
* **Testing**:
  - Comprehensive unit and integration test suites expanded with 139 passing test cases covering batch unpacking, cross-process dedup, crash traps, 3-tier escalation, and standby browser handoffs.
